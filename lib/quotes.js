...
Quotes.prototype._read = function (n, event) {
  var row = this._fetch.read();
  if (row === null) return this._readableState.reading = false;
...
Quotes.prototype._remote = function (first) {
  ...
  // Call read(0) on readable event
  this._fetch.on('readable', this._handler.readable);
